...
SELECT release_year, COUNT(release_year) AS release_count FROM `deb-01-346121.netflix_titles.netflix_titles` WHERE type='Movie' GROUP BY release_year ORDER BY release_count DESC;
...
SELECT rating, COUNT(rating) AS rating_count FROM `deb-01-346121.netflix_titles.netflix_titles` GROUP BY rating ORDER BY rating_count DESC;
SELECT country, COUNT(country) AS country_count FROM `deb-01-346121.netflix_titles.netflix_titles` GROUP BY country ORDER BY country_count DESC;

-- Using MAX: to show the most recent year on netflix that has Robert De Niro listed in the cast
SELECT Max(release_year) AS max_release FROM `deb-01-346121.netflix_titles.netflix_titles` WHERE `cast` LIKE '%Robert De Niro%';